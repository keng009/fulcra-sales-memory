...
| `/sales/handoff.md` | Durable handoff: open follow-ups, pending intros, next actions — plus the `## Vetoed keys` list (dedupe keys the user has vetoed; every read these skills perform excludes them, and no commit re-imports them; needed because typed records have no per-record delete — readers outside these skills must apply the list themselves) the `## Sweep watermarks` list (one line per swept source: `- <source>: <ISO-8601 of last completed sweep>`; advanced only after a sweep's digest is fully resolved; an interrupted or failed sweep leaves it unmoved, and parked items are never re-offered), the `## Preferences` list (durable user choices these skills honor; defined lines: `- crm-contact-creation: ask-each-time|never` (ADR-0008); `- crm: <crm>` / `- crm[<pipeline>]: <crm>` plus `crm-workspace`, `crm-pipeline`, `crm-stage-map` (written by `crm-setup`); `- auto-log: <sources>` / `- auto-log[<pipeline>]: <sources>` with sources drawn from `transcripts, calendar, email` — the standing, revocable yes for unattended sweeps, ADR-0009; `email` is opt-in per ADR-0010 and reads the mailbox read-only — and `- email-pipeline: <address>=<pipeline>` mapping a mailbox to a business for multi-pipeline sellers), and the `## Sweep log` list (one receipt line per sweep run — `- <ISO start> | sources: <list> | committed <n> | parked <n> | skipped-duplicate <n> | failed: <none|detail>` — newest 30 kept, older lines moved to `sweep-log-archive.md`), and the optional `## Pipelines` registry (one line per business the user sells for, e.g. `- fulcra`; absent or single-entry for most users — see Pipelines below). |
| `/sales/sweep-log-archive.md` | Older `## Sweep log` receipt lines, appended when `handoff.md`'s list exceeds 30. Written only by the sweep; never read for decisions. |
