...
<!-- Trigger phrases: snapshot — "show me my last 30 days", "snapshot my pipeline";
     ...
     lead intro; daily rhythm — "prep my day", "what do I owe people", "sweep", "check my inboxes", "make this automatic"; recall — "prep me for…", "what do I know about…"; reporting —
...
7. **Make it automatic — the skill builds the schedule (opt-in).** Trigger: "make this automatic", "schedule my sweeps", "auto-sync", or offered once, in one line, right after the user turns on auto-log ("Want me to run this on a schedule so you never have to say 'sweep'?"). Steps:
   ...
   - **The task's prompt must stand alone** — each run starts a fresh session with no memory of this chat. Include: which skill to load and follow (this one, Tend rules 5, 6 and 8 and the Rails) and to STOP if the skill can't be loaded rather than run from memory; the namespace and registered pipelines; the connectors to use (Fulcra; the calendar connector's own tool, not Fulcra's calendar tool; the transcript tool; the browser tools only if the user said yes to the inbox glance in rule 8); the failure playbook (dead Fulcra = stop, no writes); and the digest format in plain words. A template lives in `references/scheduling.md`.
...
8. **Inbox glance — LinkedIn DMs and WhatsApp Web, read-only, from the user's own browser (opt-in).** Trigger: "check my inboxes", "check my LinkedIn inbox", "check my WhatsApp", or the user asking why a lead's reply isn't in memory. This is the browser-observation tier in `references/messaging-capture.md`; its account-risk posture is not optional. Steps:
   - **Detect the capability, never assume it**: tools that drive the user's own logged-in browser (Claude in Chrome exposes `list_connected_browsers`, `tabs_context_mcp`, `navigate`, `get_page_text`, `find`, `read_page`; other harnesses vary). None present → say so and give the paste path ("copy the thread here and say log this"). Present → say once, the first time only, that reading an inbox this way is the user's informed choice under those platforms' terms; then proceed on their yes.
   - **Inbox only, one window, read only what is new**: open the messaging inbox, read the conversation list, open only threads newer than the `linkedin dms` / `whatsapp` watermark (a handful at most, seconds each). Never a profile, feed, search, or company page; never a send, reply, reaction, connection request, or a keystroke in a message box. A WhatsApp "scan to log in" screen means it isn't linked: skip it, say so, don't touch its watermark — linking is the user's step, never the skill's. Say plainly that opening a thread marks it seen, as a glance does.
   - **Ledger, one yes, never auto-logged**: business threads (a lead, customer, partner, host, or vendor-as-prospect in a registered pipeline) go on a ledger with person, date, gist, proposed pipeline, and the reason; investor threads, group chats, personal threads, vendor pitches, and auto-replies are named and skipped. On the yes: the standard dual write with the date-form key (`touch:<person-slug>:<date>` — a browser read yields no stable id), channel `message`, evidence `browser observation, linkedin dms <date>` or `browser observation, whatsapp <date>`; CRM note per the pipeline's mapping only where the contact already exists. Messaging is never an ADR-0009 source: inside a scheduled run the same threads are parked in the review queue for the next live yes.
   - **Stop on the first warning**: a CAPTCHA, a verification or "unusual activity" prompt, a restriction notice, or an unrecognized layout → stop, report exactly what appeared, write `- browser-observation: paused <date> — <reason>` under `## Preferences`, and skip the glance until the user removes that line. After any anomaly, skip the next scheduled glance.
   - **Receipt and watermarks**: one `## Sweep log` line naming the sources; advance `linkedin dms` and `whatsapp` only for a source that was actually read. Close the tab you opened.
   - **In the schedule**: when rule 7 builds the task and the user has said yes to the glance, the task prompt carries it as one read-only window per run (template in `references/scheduling.md`), never a second window the same day.
